...
impl Repository {
    ...
    pub fn get_pull_file(
        ...
        let mut stmt = conn.prepare("SELECT timestamp FROM file_pulls WHERE target_id=? AND file_path=?")?;
        let mut rows = stmt.query([target_id, file_path])?;
        if let Some(row) = rows.next()? {
            let timestamp: DateTime<Utc> = row.get(0)?;
            return Ok(Some((target_id.to_owned(), file_path.to_owned(), timestamp)));
        }
...
mod tests {
    ...
    fn test_new() -> Result<()> {
        ...
        let rnd = SystemTime::now()
            ...
            .unwrap()
            .as_millis();
        let tmp_db_path = std::env::temp_dir().join(format!("fsy_repo_test_{rnd}.db"));
        let tmp_db_path_str = tmp_db_path.to_str().unwrap().to_owned();

        let repo = Repository::new(tmp_db_path_str.clone());
    ...
    fn test_get_pull_file() -> Result<()> {
        ...
        // check timestamp
        let (_, _, file_timestamp) = repo
            .get_pull_file(&target_id, &file_path)
            .unwrap().unwrap();

        assert_eq!(file_timestamp.timestamp_millis(), timestamp.timestamp_millis());
